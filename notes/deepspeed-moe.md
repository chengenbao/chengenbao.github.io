...
![不同模型大小的 Dense vs MoE 验证损失曲线：MoE 模型以更少的训练计算达到同等甚至更优的收敛质量](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/moe_valid_loss_350_to_6p7.png)
*图：不同大小的 Dense 模型与 MoE 模型的验证损失曲线对比。MoE 模型在训练成本远低于 Dense 模型的情况下达到同等质量。*
...
![标准 MoE 与 PR-MoE 架构对比](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure2_DeepSpeed-scaled.jpg)
*图：标准 MoE（左）与 PR-MoE（右）架构对比。PR-MoE 采用金字塔式专家分配（深层更多专家）和残差连接（固定专家 + 动态路由专家）。*
...
![DS-MoE 多维并行设计](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/expert-parallelism-scaled.jpg)
*图：DS-MoE 针对模型的专家分区和非专家分区分别采用不同的多维并行策略（TP + EP + DP）。*
...
![分层 All-to-All 设计示意图](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure4_DeepSpeed.jpg)
*图：分层 All-to-All——先节点内局部通信（NVLink），再跨节点全局通信（InfiniBand），大幅降低通信复杂度。*
...
![并行协调通信优化示意图](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure5_DeepSpeed.png)
*图：Tensor-Expert 协同并行——利用 TP 组内输入一致性避免冗余 All-to-All。*
...
![PR-MoE + MoS 推理资源需求对比](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure7_DeepSpeed.png)
*图：使用 PR-MoE + MoS 后，MoE 推理所需的 GPU 资源减少一半。*

### 不同配置下的推理延迟

![不同 GPU 数量和模型大小下的推理延迟对比](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure8_DeepSpeed.jpg)
*图：标准 MoE 与 PR-MoE、PR-MoE + MoS 在不同 GPU 数量和模型规模下的推理延迟比较。*

### MoE vs Dense 推理延迟

![52B MoE vs 6.7B Dense 推理延迟对比](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure9_DeepSpeed.jpg)
*图：52B 参数 MoE 模型与质量相当的 6.7B Dense 模型的推理延迟对比。*

![1.5T MoE vs 175B Dense 推理延迟对比](https://www.microsoft.com/en-us/research/wp-content/uploads/2022/01/Figure10_DeepSpeed.jpg)
*图：1.5 万亿参数 MoE 模型与质量相当的 175B Dense 模型的推理延迟对比。*
